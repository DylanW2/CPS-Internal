...
public class mapSpawner : MonoBehaviour
{
    ...
    public void newMap()
    {
        ...
        width = Random.Range(minWidth, maxWidth);
        side = mG.secSide;
        oSide = Random.Range(0, 3);
        if ((side == 1 && oSide == 0) || (side == 3 && oSide == 2) || (side == 2 && oSide == 3) || (side == 0 && oSide == 1))
        {
            int tempSide = oSide;
            while (tempSide == oSide) oSide = Random.Range(0, 3);
        }
        if (side == 0)
        {
